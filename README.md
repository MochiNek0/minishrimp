...
- **本地记忆与语义路由** —— 使用 32 维语义向量路由机制，自动将对话切分为不同的话题 Session。支持跨话题上下文隔离、长效同话题回溯，所有记忆存放在本地 SPIFFS。
...
| `sessions_idx.bin`| 32 维语义向量中央索引，用于管理多话题会话       |
...
1. **语义分类**：每一条消息都会通过 LLM 映射到 32 维语义空间。
