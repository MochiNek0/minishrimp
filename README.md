...
**$5 芯片上的 AI 助理。没有 Linux，没有 Node.js，纯 C。**
...
- **工具调用与定时任务** —— 支持 ReAct Agent 循环能力，可自主调用网页搜索、查看时间、文件读写等工具；内置 Cron 调度器，AI 可自主创建每日计划和提醒
- **极低功耗** —— USB 供电，24/7 运行仅约 0.5W，比一部手机的待机功耗还低

## 工作原理

![](assets/minishrimp.png)
...
#define SHRIMP_SECRET_MODEL_PROVIDER  "openai"        // 格式兼容 OpenAI 写 openai 即可支持大多数国产模型
```
...
| `cron_add`/`list`  | 让 AI 自主设定闹钟、每日简报和备忘录循环提醒任务    |
| `files_tool`       | (对特定开发者开放) 直接检索、读取微控制器的内部路径 |

## 文档详情与开发参与

- 关于具体的架构规划，参考： [**docs/ARCHITECTURE.md**](docs/ARCHITECTURE.md) 。
- 本项目针对 `mimiclaw` 的重构涵盖了双 IM 机器人（飞书等企业平台对接）、代理分发等特性，详情可见：[**docs/im-integration/README.md**](docs/im-integration/README.md)。

## 贡献者

感谢所有为本项目做出贡献的开发者！

<a href="https://github.com/MochiNek0/minishrimp/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=MochiNek0/minishrimp" alt="Contributors" />
</a>
...
AI Agent 的实现灵感部分借助于 [OpenClaw](https://github.com/openclaw/openclaw) 与 [Nanobot](https://github.com/HKUDS/nanobot)。在 $5 的微型单片机上将 ReAct 机制落地是一次激动人心的探索。
