...
- **工具调用与定时任务** —— 支持 ReAct Agent 循环能力，可自主调用网页搜索、网页抓取、日历查询、天气查询、文件读写等工具；内置 Cron 调度器，AI 可自主创建每日计划和提醒
- **通用 API 调用** —— 内置 `api_call` 工具，支持任意 REST API 调用，结合动态技能可运行时扩展任意能力
- **日期与时间感知** —— 系统原生注入实时时间锚点与 7 天参考日历，支持精确的短/长期日期推算与日程规划能力
- **多模态支持** —— 支持图片消息收发，可处理用户发送的图片并生成图文回复
...
│   ├── agent/                # Agent 循环、上下文构建、语义路由器
│   │   ├── agent_loop.c      # ReAct Agent 主循环
│   │   ├── context_builder.c # 对话上下文构建
│   │   ├── subject_router.c  # 64 维语义向量话题路由
│   │   └── subject_router.h
│   ├── bus/                  # 消息总线（入站/出站队列）
...
│   ├── skills/               # 技能加载器
│   ├── tools/                # 工具实现（搜索、天气、日历、文件、API 调用等）
│   │   ├── tool_api.c/h      # 通用 API 调用工具
│   │   ├── tool_calendar.c/h # 日历查询工具
│   │   └── ...
│   ├── utils/                # 通用工具函数（含 JSON 修复）
│   └── wifi/                 # WiFi 管理器
...
5. **消息防抖**：基于语义向量变化的阈值检测，防止短时间内的重复消息重复创建话题。
6. **JSON 自动修复**：记忆存储支持自动修复损坏的 JSON，内建回退机制保障数据完整性。

## 工具与技能支持

MiniShrimp 支持**两层**可扩展机制：

### 第一层：预置工具（编译时固定，高效专用）

MiniShrimp 内置以下工具，通过 OpenAI 标准的 Function Calling 协议调用：

| 工具名                                          | 用途                                                    |
| ----------------------------------------------- | ------------------------------------------------------- |
| `api_call`                                      | 通用 HTTP API 调用器，支持 GET/POST/PUT/DELETE、Bearer 认证、自定义 Header 与 JSON Body |
| `web_search`                                    | 通过 Tavily 或 Brave API 搜索互联网，回答时效性问题     |
| `web_fetch`                                     | 抓取网页内容并提取纯文本，支持代理和 Jina Reader 后备   |
| `get_weather`                                   | 获取指定城市的当前天气或多日天气预报                    |
| `get_calendar`                                  | 获取指定月份的日历视图，用于精确日期核对与规划          |
| `cron_add`/`list`/`remove`                      | 让 AI 自主设定闹钟、每日简报和备忘录循环提醒任务        |
| `read_file`/`write_file`/`edit_file`/`list_dir` | (对特定开发者开放) 直接检索、读写微控制器的内部文件系统 |

这些工具在固件烧录时即固定，针对高频场景进行了封装优化（如 `web_fetch` 内部自动处理 Jina Reader 后备、HTML 清洗、重试逻辑），每次调用仅需一次工具调用。

### 第二层：动态技能（运行时可扩展，无需重烧）

除了预置工具外，MiniShrimp 还支持**运行时动态创建技能**。每个技能是一个 Markdown 文件，存储在 SPIFFS 的 `/skills/` 目录下，内含 AI  Agent 的专用指令和调用规范。

**核心优势：烧录后仍可通过聊天创建新技能。** 例如，要让 AI 学会查询 GitHub 仓库信息，只需让 AI 自己创建一个 `github-repo.md`：
...
## 何时使用
用户询问某个 GitHub 仓库的 stars、forks、issues 等信息时使用。
...
创建后，AI 立刻学会调用 GitHub API，无需修改代码、重新编译或烧录。

**`api_call` 工具**是动态扩展的核心——它是一个通用 HTTP 客户端，支持任意 REST API 调用。结合自定义 skill，理论上可以在运行时让 AI 掌握任何有 HTTP API 的服务。

> **提示**：技能文件存储在 SPIFFS 中，通过 `edit_file` 工具由 AI 自主创建和修改。
...
1. 让 AI 直接用 `edit_file` 在 SPIFFS 的 `/skills/` 目录下创建或编辑 `.md` 技能文件
2. 在技能中描述清楚何时使用、如何使用现有工具（如 `api_call`）完成目标
3. 重启后技能自动加载，AI 在 system prompt 中即可看到新技能

示例：创建一个"股票查询"技能，只需告诉 AI 创建 `/skills/stock.md`，在里面写清楚怎么用 `api_call` 调用股票 API，AI 马上就能用。

**方式二：编译时添加工具（面向高性能专用场景）**

如果某个功能需要高效封装、减少工具调用次数、或涉及复杂的状态管理，在源码层面添加专用工具：
...
> **区别**：预置工具（如 `web_fetch`、`get_weather`）在固件内部处理复杂逻辑（如自动重试、结果清洗、多 API 串联），每次调用只需一次工具调用。动态技能则依赖 `api_call` 进行裸 HTTP 调用，更灵活但可能需要多次调用才能完成一个任务。
