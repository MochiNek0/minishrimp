...
{
    s_inbound_queue = xQueueCreate(SHRIMP_BUS_QUEUE_LEN, sizeof(shrimp_msg_t));
    s_outbound_queue = xQueueCreate(SHRIMP_BUS_QUEUE_LEN, sizeof(shrimp_msg_t));
    ...
    ESP_LOGI(TAG, "Message bus initialized (queue depth %d)", SHRIMP_BUS_QUEUE_LEN);
    return ESP_OK;
}
