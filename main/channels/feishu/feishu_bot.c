...
static char s_app_secret[128] = SHRIMP_SECRET_FEISHU_APP_SECRET;
...
{
    nvs_handle_t nvs;
    if (nvs_open(SHRIMP_NVS_FEISHU, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp_id[64] = {0};
        char tmp_secret[128] = {0};
        size_t len_id = sizeof(tmp_id);
        size_t len_secret = sizeof(tmp_secret);

        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_FEISHU_APP_ID, tmp_id, &len_id) == ESP_OK && tmp_id[0]) {
            strncpy(s_app_id, tmp_id, sizeof(s_app_id) - 1);
        }
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_FEISHU_APP_SECRET, tmp_secret, &len_secret) == ESP_OK && tmp_secret[0]) {
    ...
    if (s_app_id[0] && s_app_secret[0]) {
        ...
        ESP_LOGW(TAG, "No Feishu credentials. Configure via web UI.");
    }
...
esp_err_t feishu_bot_start(void)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(
        feishu_ws_task,
        "feishu_ws",
        SHRIMP_FEISHU_POLL_STACK,
        NULL,
        SHRIMP_FEISHU_POLL_PRIO,
        &s_ws_task,
        SHRIMP_FEISHU_POLL_CORE);
    if (ok != pdPASS) {
        s_ws_task = NULL;
        return ESP_FAIL;
    }
...
esp_err_t feishu_send_message(const char *chat_id, const char *text)
{
    ...
    while (offset < text_len) {
        ...
        if (chunk > SHRIMP_FEISHU_MAX_MSG_LEN) {
            chunk = SHRIMP_FEISHU_MAX_MSG_LEN;
        }
