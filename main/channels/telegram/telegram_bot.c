...
static char s_bot_token[128] = SHRIMP_SECRET_TG_TOKEN;
...
static void save_update_offset_if_needed(bool force)
{
    ...
    nvs_handle_t nvs;
    if (nvs_open(SHRIMP_NVS_TG, NVS_READWRITE, &nvs) != ESP_OK) {
        return;
    }
...
static char *tg_api_call_via_proxy(const char *path, const char *post_data)
{
    proxy_conn_t *conn = proxy_conn_open("api.telegram.org", 443,
                                          (SHRIMP_TG_POLL_TIMEOUT_S + 5) * 1000);
    if (!conn) return NULL;
    ...
    int timeout = (SHRIMP_TG_POLL_TIMEOUT_S + 5) * 1000;
    while (1) {
        if (len + 1024 >= cap) {
            cap *= 2;
...
static char *tg_api_call_direct(const char *method, const char *post_data)
{
    ...
    esp_http_client_config_t config = {
        ...
        .user_data = &resp,
        .timeout_ms = (SHRIMP_TG_POLL_TIMEOUT_S + 5) * 1000,
        .buffer_size = 2048,
        .buffer_size_tx = 2048,
...
static void telegram_poll_task(void *arg)
{
    ...
    while (1) {
        ...
        snprintf(params, sizeof(params),
                 ...
                 s_update_offset, SHRIMP_TG_POLL_TIMEOUT_S);
...
esp_err_t telegram_bot_init(void)
{
    ...
    nvs_handle_t nvs;
    if (nvs_open(SHRIMP_NVS_TG, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[128] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_TG_TOKEN, tmp, &len) == ESP_OK && tmp[0]) {
    ...
    /* s_bot_token is already initialized from SHRIMP_SECRET_TG_TOKEN as fallback */

    if (s_bot_token[0]) {
        ESP_LOGI(TAG, "Telegram bot token loaded (len=%d)", (int)strlen(s_bot_token));
    } else {
        ESP_LOGW(TAG, "No Telegram bot token. Configure via web UI.");
    }
    return ESP_OK;
}
...
{
    if (s_bot_token[0] == '\0') {
        ESP_LOGW(TAG, "Telegram not configured, skipping poll start");
        return ESP_OK;
    }
    if (s_poll_task) {
        ESP_LOGW(TAG, "Telegram poll task already running");
        return ESP_OK;
    }
    BaseType_t ret = xTaskCreatePinnedToCore(
        telegram_poll_task, "tg_poll",
        SHRIMP_TG_POLL_STACK, NULL,
        SHRIMP_TG_POLL_PRIO, &s_poll_task, SHRIMP_TG_POLL_CORE);
...
esp_err_t telegram_send_message(const char *chat_id, const char *text)
{
    ...
    while (offset < text_len) {
        ...
        if (chunk > SHRIMP_TG_MAX_MSG_LEN) {
            chunk = SHRIMP_TG_MAX_MSG_LEN;
        }
