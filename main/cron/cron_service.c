...
#define MAX_CRON_JOBS  SHRIMP_CRON_MAX_JOBS
...
static bool cron_sanitize_destination(cron_job_t *job)
{
    ...
    if (job->channel[0] == '\0') {
        strncpy(job->channel, SHRIMP_CHAN_SYSTEM, sizeof(job->channel) - 1);
        changed = true;
    }

    if (strcmp(job->channel, SHRIMP_CHAN_TELEGRAM) == 0 || strcmp(job->channel, SHRIMP_CHAN_FEISHU) == 0) {
...
{
    FILE *f = fopen(SHRIMP_CRON_FILE, "r");
    if (!f) {
    ...
    cJSON_ArrayForEach(item, jobs_arr) {
        ...
        strncpy(job->channel, channel ? channel : SHRIMP_CHAN_SYSTEM,
...
static esp_err_t cron_save_jobs(void)
{
    ...
    ESP_LOGI(TAG, "Saved %d cron jobs to %s", s_job_count, SHRIMP_CRON_FILE);
    return ESP_OK;
}
...
static void cron_task_main(void *arg)
{
    ...
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(SHRIMP_CRON_CHECK_INTERVAL_MS));
...
esp_err_t cron_service_start(void)
{
    ...
    ESP_LOGI(TAG, "Cron service started (%d jobs, check every %ds)",
             s_job_count, SHRIMP_CRON_CHECK_INTERVAL_MS / 1000);
    return ESP_OK;
}
