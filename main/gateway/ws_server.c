...
static ws_client_t s_clients[SHRIMP_WS_MAX_CLIENTS];

static ws_client_t *find_client_by_fd(int fd)
{
    for (int i = 0; i < SHRIMP_WS_MAX_CLIENTS; i++) {
...
static esp_err_t ws_handler(httpd_req_t *req)
{
    ...
    if (type && cJSON_IsString(type) && strcmp(type->valuestring, "message") == 0
        ...
        /* Push to inbound bus */
        shrimp_msg_t msg = {0};
        strncpy(msg.channel, SHRIMP_CHAN_WEBSOCKET, sizeof(msg.channel) - 1);
...
esp_err_t ws_server_start(void)
{
    ...
    config.server_port = SHRIMP_WS_PORT;
    config.ctrl_port = SHRIMP_WS_PORT + 1;
    config.max_open_sockets = SHRIMP_WS_MAX_CLIENTS;
    ...
    web_config_register(s_server);

    ESP_LOGI(TAG, "WebSocket server started on port %d", SHRIMP_WS_PORT);
    return ESP_OK;
}
