...
static char s_model[LLM_MODEL_MAX_LEN] = SHRIMP_LLM_DEFAULT_MODEL;
static char s_provider[16] = SHRIMP_LLM_PROVIDER_DEFAULT;
static char s_custom_url[256] = SHRIMP_SECRET_CUSTOM_URL;
static char s_custom_header[64] = SHRIMP_SECRET_CUSTOM_HEADER;
static char s_custom_prefix[32] = SHRIMP_SECRET_CUSTOM_PREFIX;
...
#if SHRIMP_LLM_LOG_VERBOSE_PAYLOAD
...
#else
    if (SHRIMP_LLM_LOG_PREVIEW_BYTES > 0) {
        size_t shown = total > SHRIMP_LLM_LOG_PREVIEW_BYTES ? SHRIMP_LLM_LOG_PREVIEW_BYTES : total;
        char preview[SHRIMP_LLM_LOG_PREVIEW_BYTES + 1];
...
{
    const char *url = provider_base_url();
    if (url && url[0]) return url;
    return provider_is_openai_compatible() ? SHRIMP_OPENAI_API_URL : SHRIMP_LLM_API_URL;
}

static const char *custom_header_name(void)
{
    return s_custom_header[0] ? s_custom_header : "Authorization";
}

static const char *custom_header_prefix(void)
{
    return s_custom_prefix[0] ? s_custom_prefix : "Bearer ";
}
...
static void llm_apply_auth_header(esp_http_client_handle_t client)
{
    if (provider_is_openai_compatible()) {
        if (s_api_key[0]) {
            if (provider_requires_custom_url()) {
                char auth[LLM_API_KEY_MAX_LEN + 64];
                snprintf(auth, sizeof(auth), "%s%s", custom_header_prefix(), s_api_key);
                esp_http_client_set_header(client, custom_header_name(), auth);
            } else {
        ...
        esp_http_client_set_header(client, "anthropic-version", SHRIMP_LLM_API_VERSION);
    }
}
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (SHRIMP_SECRET_API_KEY[0] != '\0') {
        safe_copy(s_api_key, sizeof(s_api_key), SHRIMP_SECRET_API_KEY);
    }
    if (SHRIMP_SECRET_MODEL[0] != '\0') {
        safe_copy(s_model, sizeof(s_model), SHRIMP_SECRET_MODEL);
    }
    if (SHRIMP_SECRET_MODEL_PROVIDER[0] != '\0') {
        safe_copy(s_provider, sizeof(s_provider), SHRIMP_SECRET_MODEL_PROVIDER);
    }

    /* NVS overrides take highest priority (set via web UI) */
    nvs_handle_t nvs;
    if (nvs_open(SHRIMP_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp[LLM_API_KEY_MAX_LEN] = {0};
        size_t len = sizeof(tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_API_KEY, tmp, &len) == ESP_OK && tmp[0]) {
            safe_copy(s_api_key, sizeof(s_api_key), tmp);
        }
        char model_tmp[LLM_MODEL_MAX_LEN] = {0};
        len = sizeof(model_tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_MODEL, model_tmp, &len) == ESP_OK && model_tmp[0]) {
            safe_copy(s_model, sizeof(s_model), model_tmp);
        }
        char provider_tmp[16] = {0};
        len = sizeof(provider_tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_PROVIDER, provider_tmp, &len) == ESP_OK && provider_tmp[0]) {
            safe_copy(s_provider, sizeof(s_provider), provider_tmp);
        }
        char custom_url_tmp[256] = {0};
        len = sizeof(custom_url_tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_CUSTOM_URL, custom_url_tmp, &len) == ESP_OK && custom_url_tmp[0]) {
            safe_copy(s_custom_url, sizeof(s_custom_url), custom_url_tmp);
        }
        char custom_header_tmp[64] = {0};
        len = sizeof(custom_header_tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_CUSTOM_HEADER, custom_header_tmp, &len) == ESP_OK && custom_header_tmp[0]) {
            safe_copy(s_custom_header, sizeof(s_custom_header), custom_header_tmp);
        }
        char custom_prefix_tmp[32] = {0};
        len = sizeof(custom_prefix_tmp);
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_CUSTOM_PREFIX, custom_prefix_tmp, &len) == ESP_OK && custom_prefix_tmp[0]) {
            safe_copy(s_custom_prefix, sizeof(s_custom_prefix), custom_prefix_tmp);
        }
        nvs_close(nvs);
    }
    ...
    if (s_api_key[0]) {
        ...
        ESP_LOGW(TAG, "No API key. Configure via web UI.");
    }
    return ESP_OK;
}
...
static esp_err_t llm_http_via_proxy(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    char path_part[192];
    int port = 443;
    if (!parse_url_parts(llm_api_url(), host, sizeof(host), path_part, sizeof(path_part), &port)) {
    ...
    int hlen = 0;
    if (provider_is_openai_compatible()) {
        const char *hname = (provider_requires_custom_url()) ? custom_header_name() : "Authorization";
        const char *prefix = (provider_requires_custom_url()) ? custom_header_prefix() : "Bearer ";
        hlen = snprintf(header, sizeof(header),
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            path_part, host, hname, prefix, s_api_key, body_len);
    } else {
        hlen = snprintf(header, sizeof(header),
            "POST %s HTTP/1.1\r\n"
            "Host: %s\r\n"
            ...
            "Content-Length: %d\r\n"
            "Connection: close\r\n\r\n",
            path_part, host, s_api_key, SHRIMP_LLM_API_VERSION, body_len);
    }
                         ...
                         llm_response_t *resp)
{
    ...
    if (provider_requires_custom_url() && s_custom_url[0] == '\0') {
        ESP_LOGW(TAG, "Provider %s requires custom URL, but it is empty", s_provider);
    ...
    if (provider_is_openai_compatible()) {
        cJSON_AddNumberToObject(body, "max_completion_tokens", SHRIMP_LLM_MAX_TOKENS);
    } else {
        cJSON_AddNumberToObject(body, "max_tokens", SHRIMP_LLM_MAX_TOKENS);
    }
    ...
    /* HTTP call */
    resp_buf_t rb;
    if (resp_buf_init(&rb, SHRIMP_LLM_STREAM_BUF_SIZE) != ESP_OK) {
        free(post_data);
            ...
            if (message) {
                ...
                if (tool_calls && cJSON_IsArray(tool_calls)) {
                    ...
                    cJSON_ArrayForEach(tc, tool_calls) {
                        if (resp->call_count >= SHRIMP_MAX_TOOL_CALLS) break;
...
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_API_KEY, api_key));
...
esp_err_t llm_set_model(const char *model)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_MODEL, model));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_provider(const char *provider)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_PROVIDER, provider));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_custom_url(const char *url)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_CUSTOM_URL, url));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_custom_header(const char *header_name)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_CUSTOM_HEADER, header_name));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
...
esp_err_t llm_set_custom_prefix(const char *prefix)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_LLM, NVS_READWRITE, &nvs));
    ESP_ERROR_CHECK(nvs_set_str(nvs, SHRIMP_NVS_KEY_CUSTOM_PREFIX, prefix));
    ESP_ERROR_CHECK(nvs_commit(nvs));
    nvs_close(nvs);
