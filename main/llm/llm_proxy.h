...
/**
 * Save the LLM provider to NVS. (e.g. "anthropic", "openai", "qwen", "gemini", "deepseek", "zhipu", "moonshot", "minimax", "yi", "doubao", "hunyuan", "baichuan", "qianfan", "spark", "custom")
 */
...
/**
 * Save custom API URL (OpenAI-compatible) to NVS.
 */
esp_err_t llm_set_custom_url(const char *url);

/**
 * Save custom auth header name to NVS.
 */
esp_err_t llm_set_custom_header(const char *header_name);

/**
 * Save custom auth header prefix to NVS (e.g. "Bearer ").
 */
...
typedef struct {
    ...
    size_t text_len;
    llm_tool_call_t calls[SHRIMP_MAX_TOOL_CALLS];
    int call_count;
