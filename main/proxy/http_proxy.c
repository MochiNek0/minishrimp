...
esp_err_t http_proxy_init(void)
{
    ...
    if (SHRIMP_SECRET_PROXY_HOST[0] != '\0' && SHRIMP_SECRET_PROXY_PORT[0] != '\0') {
        strncpy(s_proxy_host, SHRIMP_SECRET_PROXY_HOST, sizeof(s_proxy_host) - 1);
        s_proxy_port = (uint16_t)atoi(SHRIMP_SECRET_PROXY_PORT);
        if (SHRIMP_SECRET_PROXY_TYPE[0] != '\0') {
            strncpy(s_proxy_type, SHRIMP_SECRET_PROXY_TYPE, sizeof(s_proxy_type) - 1);
        }
    }
    ...
    if (nvs_open(SHRIMP_NVS_PROXY, NVS_READONLY, &nvs) == ESP_OK) {
        ...
        if (nvs_get_str(nvs, SHRIMP_NVS_KEY_PROXY_HOST, tmp, &len) == ESP_OK && tmp[0]) {
...
esp_err_t http_proxy_clear(void)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(SHRIMP_NVS_PROXY, NVS_READWRITE, &nvs));
    nvs_erase_key(nvs, SHRIMP_NVS_KEY_PROXY_HOST);
    nvs_erase_key(nvs, SHRIMP_NVS_KEY_PROXY_PORT);
