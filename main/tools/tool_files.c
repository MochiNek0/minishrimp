...
/**
 * Validate that a path starts with SHRIMP_SPIFFS_BASE and contains no ".." traversal.
 */
static bool validate_path(const char *path)
{
    if (!path) return false;
    size_t base_len = strlen(SHRIMP_SPIFFS_BASE);
    if (strncmp(path, SHRIMP_SPIFFS_BASE, base_len) != 0) return false;
    /* Require a path separator after the base (unless base ends with '/') */
    if (base_len > 0 && SHRIMP_SPIFFS_BASE[base_len - 1] != '/') {
...
esp_err_t tool_read_file_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    if (!validate_path(path)) {
        snprintf(output, output_size, "Error: path must start with %s/ and must not contain '..'", SHRIMP_SPIFFS_BASE);
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t tool_list_dir_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    DIR *dir = opendir(SHRIMP_SPIFFS_BASE);
    if (!dir) {
        snprintf(output, output_size, "Error: cannot open %s directory", SHRIMP_SPIFFS_BASE);
        cJSON_Delete(root);
        return ESP_FAIL;
    }
    ...
    while ((ent = readdir(dir)) != NULL && off < output_size - 1) {
        ...
        char full_path[512];
        snprintf(full_path, sizeof(full_path), "%s/%s", SHRIMP_SPIFFS_BASE, ent->d_name);
